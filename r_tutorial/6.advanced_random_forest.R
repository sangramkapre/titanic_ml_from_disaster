...
# retrieve cabin deck
combined$charCabin <- as.character(combined$Cabin)
combined$Deck <- sapply(combined$charCabin, FUN = function(x) {substring(x, 1, 1)})
combined$Deck[combined$Deck == ""] <- "X"
combined$Deck <- as.factor(combined$Deck)

# retrive cabin side (based on last digit of location number)
combined$cabin.last.digit <- str_sub(data$Cabin, -1)
combined$Side <- "UNK"
combined$Side[which(isEven(combined$cabin.last.digit))] <- "port"
combined$Side[which(isOdd(combined$cabin.last.digit))] <- "starboard"
combined$Side <- as.factor(combined$Side)
combined$cabin.last.digit <- NULL
#####################################
...
fit <- randomForest(as.factor(Survived) ~ Pclass+Sex+Age+SibSp+Parch+Fare+
                        Embarked+Title+FamilySize+FamilyId2+Side,
...
write.csv(submit, file = "advancedRandomForestModel.csv", row.names = FALSE)
